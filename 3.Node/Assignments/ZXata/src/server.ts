...
// app.put('/user/update/:name', async (req: Request, res: Response) => {
//     const personName: string = req.params.name.toLowerCase();
//     const { email, password, name } = req.body;

//     if (!email && !password && !name) {
//         return res.status(400).json({ message: 'At least one field (name, email, or password) is required for an update.' });
//     }
...
//             password: password ?? foundPerson.password, // Consider hashing the password
//             name: name ?? foundPerson.name
//         };

//         // Update the person’s information in the Xata database
//         await client.db.personInfo.update(foundPerson.id, updatedPersonData);
//         res.status(200).json({ message: 'User updated successfully', updatedPersonData });
        
//     } catch (error) {
//         console.error(`Error updating data for person ${personName}: ${error}`);
